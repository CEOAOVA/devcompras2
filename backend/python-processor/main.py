...
async def get_product_statistics(
    ...
    advanced_filter: Optional[str] = Query(None)  # JSON string for advanced filtering
):
    ...
    try:
        ...
        if advanced_filter:
            try:
                import json
                advanced_config = json.loads(advanced_filter)
                logger.info(f"Using advanced filter config")
            except (json.JSONDecodeError, TypeError) as e:
                logger.warning(f"Invalid advanced_filter format: {e}")
        ...
        # Parse store IDs if provided (for legacy filtering)
        store_ids = None
        if stores and not advanced_config:
            try:
        ...
        # Store advanced config for later use
        advanced_store_ids = None
        if advanced_config and 'stores' in advanced_config:
            selected_stores = list(advanced_config['stores'].keys())
            if selected_stores:
                advanced_store_ids = [int(sid) for sid in selected_stores]
                logger.info(f"Advanced filter: processing {len(advanced_store_ids)} stores")
        ...
        # Apply advanced filter store restriction using multiple queries (Supabase .in_() not working)
        if advanced_store_ids:
            logger.info(f"Advanced filter: querying {len(advanced_store_ids)} stores individually")
            all_results = []
            
            for store_id in advanced_store_ids:
                # Create individual query for each store
                store_query = db.admin_client.table('transactions').select('''
                    product_id,
                    store_id,
                    quantity,
                    type,
                ...
                if type:
                    normalized_type = normalize_type_filter(type)
                    store_query = store_query.eq('type', normalized_type)
                
                # Apply date filters
                if date_from and date_to:
                    store_query = store_query.gte('business_date_start', date_from).lte('business_date_end', date_to)
                elif date_from:
                    store_query = store_query.gte('business_date_end', date_from)
                elif date_to:
                    store_query = store_query.lte('business_date_start', date_to)
                elif date:
                    store_query = store_query.lte('business_date_start', date).gte('business_date_end', date)
                
                # Apply store filter
                store_query = store_query.eq('store_id', store_id)
                
                store_result = store_query.execute()
                all_results.extend(store_result.data)
            ...
            result = FakeResult(all_results)
            logger.info(f"Advanced filter: combined {len(result.data)} transactions")
        else:
            result = query.execute()
        
        logger.info(f"Found {len(result.data)} transactions for statistics")
        
        # If no transactions found and we're looking for specific stores, return empty
        if advanced_config and len(result.data) == 0:
            logger.warning("No transactions found for advanced filter stores")
            return []
        ...
        # Process data to create statistics with both sales and inventory
        product_stats = {}
        ...
        for row in result.data:
            ...
            if store_id not in product_stats[product_code]['store_inventory']:
                product_stats[product_code]['store_inventory'][store_id] = 0
            
            # Add to appropriate category based on transaction type
            if transaction_type == 'sale':
                product_stats[product_code]['store_sales'][store_id] += quantity
            elif transaction_type == 'inventory':
                product_stats[product_code]['store_inventory'][store_id] += quantity
        
        # Filter results based on advanced configuration
        final_stats = []
        logger.info(f"Processing {len(product_stats)} products")
        
        for product_code, stats in product_stats.items():
            if advanced_config and 'stores' in advanced_config:
                # Filter store data based on advanced configuration
                filtered_sales = {}
                filtered_inventory = {}
                ...
                for store_id_str, store_config in advanced_config['stores'].items():
                    ...
                    if store_config.get('sales', False):
                        raw_value = stats['store_sales'].get(store_id, 0)
                        filtered_sales[store_id] = raw_value
                        
                    
                    if store_config.get('inventory', False):
                        raw_value_inv = stats['store_inventory'].get(store_id, 0)
                        filtered_inventory[store_id] = raw_value_inv
                        
                
                
                # Only include products that have data in the selected stores/types
                if filtered_sales or filtered_inventory:
                    final_stats.append({
                        'product_code': stats['product_code'],
                        'product_name': stats['product_name'],
                        'store_sales': filtered_sales,
                        'store_inventory': filtered_inventory
                    })
            else:
                # Legacy mode: return all data
                final_stats.append(stats)
        
        logger.info(f"Returning {len(final_stats)} product statistics")
        return final_stats
...
async def export_statistics_excel(
    ...
    """
    try:
        logger.info(f"Exporting statistics to Excel with filters")
        
        db = create_database_manager()
        db.connect()
        
        # Parse advanced filter if provided (same logic as statistics endpoint)
        advanced_config = None
        if advanced_filter:
            try:
                import json
                advanced_config = json.loads(advanced_filter)
                logger.info(f"Excel export using advanced filter config")
            except (json.JSONDecodeError, TypeError) as e:
                logger.warning(f"Excel export invalid advanced_filter format: {e}")
        ...
        # Store advanced config for later use (same fix as statistics endpoint)
        excel_advanced_store_ids = None
        if advanced_config and 'stores' in advanced_config:
            selected_stores = list(advanced_config['stores'].keys())
            if selected_stores:
                excel_advanced_store_ids = [int(sid) for sid in selected_stores]
                logger.info(f"Excel export: processing {len(excel_advanced_store_ids)} stores")
        ...
        # Apply advanced filter store restriction using multiple queries (same as statistics endpoint)
        if excel_advanced_store_ids:
            logger.info(f"Excel export: querying stores individually")
            all_results = []
            
            for store_id in excel_advanced_store_ids:
                # Create individual query for each store
                store_query = db.admin_client.table('transactions').select('''
                    product_id,
                    store_id,
                ...
                # Apply date filters (Excel uses different date filtering)
                if date_from and date_to:
                    store_query = store_query.gte('created_at', f'{date_from}T00:00:00+00:00').lte('created_at', f'{date_to}T23:59:59+00:00')
                elif date_from:
                    store_query = store_query.gte('created_at', f'{date_from}T00:00:00+00:00')
                elif date_to:
                    store_query = store_query.lte('created_at', f'{date_to}T23:59:59+00:00')
                elif date:
                    store_query = store_query.gte('created_at', f'{date}T00:00:00+00:00').lte('created_at', f'{date}T23:59:59+00:00')
            ...
            result = FakeResult(all_results)
            logger.info(f"Excel export: combined {len(result.data)} transactions")
        else:
            result = query.execute()
        ...
        # Apply advanced filtering if provided (same logic as statistics endpoint)
        final_stats = []
        for product_code, stats in product_stats.items():
            if advanced_config and 'stores' in advanced_config:
                # Filter store data based on advanced configuration
                ...
                for store_id_str, store_config in advanced_config['stores'].items():
                    ...
                    if store_config.get('sales', False):
                        filtered_sales[store_id] = stats['store_sales'].get(store_id, 0)
                    
                    if store_config.get('inventory', False):
                        filtered_inventory[store_id] = stats['store_inventory'].get(store_id, 0)
        ...
        filters_info = {
            ...
            'advanced_filter': advanced_config  # Pass advanced config to Excel exporter
        }
