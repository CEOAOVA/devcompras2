...
// Tabla a explorar (cambiar según necesidad)
const TABLE_NAME = process.argv[2] || 'ARTICULOS';
const LIMIT = process.argv[3] || 5;

console.log(`\n🔍 Explorando tabla: ${TABLE_NAME}\n`);
console.log('='.repeat(80));

// Paso 1: Obtener estructura de la tabla (columnas)
const sqlColumns = `
  SELECT
...
firebird.query(sqlColumns, (err, columns) => {
  ...
  const sqlData = `SELECT FIRST ${LIMIT} * FROM ${TABLE_NAME}`;
  ...
  firebird.query(sqlData, (err, rows) => {
    ...
    if (rows.length === 0) {
      console.log('⚠️  La tabla está vacía (sin registros)');
      process.exit(0);
    }
    ...
    console.log(`\n💡 Para explorar otra tabla:`);
    console.log(`   node test/explore-table.js NOMBRE_TABLA [limite]`);
    console.log(`\n   Ejemplos:`);
    console.log(`   node test/explore-table.js CLIENTES 10`);
    console.log(`   node test/explore-table.js DOCTOS_VE 5`);
    console.log(`   node test/explore-table.js ALMACENES`);
